...
class AnticoagulationManager(Manager):
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .filter(medhistorytype=MedHistoryTypes.ANTICOAGULATION)
        )
...
class ColchicineinteractionManager(Manager):
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .filter(medhistorytype=MedHistoryTypes.COLCHICINEINTERACTION)
        )
...
class GastricbypassManager(Manager):
    def get_queryset(self):
        return (
            super().get_queryset().filter(medhistorytype=MedHistoryTypes.GASTRICBYPASS)
        )
...
class HypertensionManager(Manager):
    def get_queryset(self):
        return (
            super().get_queryset().filter(medhistorytype=MedHistoryTypes.HYPERTENSION)
        )
...
class HyperuricemiaManager(Manager):
    def get_queryset(self):
        return (
            super().get_queryset().filter(medhistorytype=MedHistoryTypes.HYPERURICEMIA)
        )
...
class OrgantransplantManager(Manager):
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .filter(medhistorytype=MedHistoryTypes.ORGANTRANSPLANT)
        )
...
class OsteoporosisManager(Manager):
    def get_queryset(self):
        return (
            super().get_queryset().filter(medhistorytype=MedHistoryTypes.OSTEOPOROSIS)
        )
...
class XoiinteractionManager(Manager):
    def get_queryset(self):
        return (
            super().get_queryset().filter(medhistorytype=MedHistoryTypes.XOIINTERACTION)
        )
